...
This document explains the C# support in Structorium in plain language.

It covers:

- What the C# module does
- How scan phases work
- What each file in `structorium/languages/csharp/` is responsible for
...
The C# module lets Structorium scan `.cs` codebases and report maintainability issues.
...
- `structorium/languages/csharp/__init__.py`
- `structorium/languages/csharp/commands.py`
- `structorium/languages/csharp/extractors.py`
- `structorium/languages/csharp/phases.py`
- `structorium/languages/csharp/detectors/deps.py`
- `structorium/languages/csharp/detectors/security.py`
- `structorium/languages/csharp/detectors/__init__.py`
- `structorium/languages/csharp/fixers/__init__.py`
...
Main config class: `structorium/languages/csharp/__init__.py`.
...
```bash
structorium --lang csharp scan --path <repo>
```
...
- Optional Roslyn JSON graph input via `STRUCTORIUM_CSHARP_ROSLYN_CMD`
...
- If no runtime option is provided, `STRUCTORIUM_CSHARP_ROSLYN_CMD` is used.
...
- Guardrails:
  - `STRUCTORIUM_CSHARP_ROSLYN_TIMEOUT_SECONDS` (default `20`)
  - `STRUCTORIUM_CSHARP_ROSLYN_MAX_OUTPUT_BYTES` (default `5242880`)
  - `STRUCTORIUM_CSHARP_ROSLYN_MAX_EDGES` (default `200000`)
- Supported JSON shapes:
  - `{"files":[{"file":"<abs-or-rel>", "imports":["<abs-or-rel>", ...]}]}`
  - `{"edges":[{"source":"<abs-or-rel>", "target":"<abs-or-rel>"}]}`
- If command execution fails or JSON is invalid, Structorium falls back to heuristic namespace/project graph building.

Example:

```bash
structorium --lang csharp detect deps --path . \
...
```bash
structorium --lang csharp detect deps --path .
structorium --lang csharp detect cycles --path .
structorium --lang csharp detect orphaned --path .
structorium --lang csharp detect dupes --path .
structorium --lang csharp detect large --path .
structorium --lang csharp detect complexity --path .
```
...
`structorium/languages/__init__.py`:
...
`structorium/languages/_framework/base/` (notably `types.py` and `shared_phases.py`):
...
`structorium/commands/detect.py` and `structorium/commands/helpers/lang.py`:

- Use dynamic language lists so C# appears in validation and error messaging

### Move command

`structorium/commands/move/move.py`:
...
`structorium/detectors/test_coverage_mapping.py`:
...
- `structorium/tests/lang/csharp/test_csharp_init.py`
- `structorium/tests/lang/csharp/test_csharp_commands.py`
- `structorium/tests/lang/csharp/test_csharp_extractors.py`
- `structorium/tests/lang/csharp/test_csharp_deps.py`
- `structorium/tests/lang/csharp/test_csharp_scan.py`
- `structorium/tests/lang/csharp/test_test_coverage_csharp.py`

Fixtures are in:

- `structorium/tests/fixtures/csharp/simple_app/`
- `structorium/tests/fixtures/csharp/multi_project/`
- `structorium/tests/fixtures/csharp/cyclic/`
- `structorium/tests/fixtures/csharp/signal_rich/`
...
- No built-in Roslyn resolver binary ships with Structorium
...
When adding behavior, always add tests in `structorium/tests/lang/csharp/test_csharp_*.py` and, if needed, a focused fixture under `structorium/tests/fixtures/csharp/`.
...
```bash
structorium --lang csharp scan --path .
structorium --lang csharp scan --profile full --path .
structorium --lang csharp scan --path . --lang-opt "roslyn_cmd=dotnet run --project /path/to/your/RoslynGraphEmitter.csproj -- {path}"
structorium --lang csharp status
structorium --lang csharp next
structorium --lang csharp show .
structorium --lang csharp detect deps --path .
structorium --lang csharp detect deps --path . --lang-opt "roslyn_cmd=dotnet run --project /path/to/your/RoslynGraphEmitter.csproj -- {path}"
structorium --lang csharp detect cycles --path .
structorium --lang csharp detect orphaned --path .
structorium --lang csharp detect dupes --path .
```
...
The C# module follows the same architecture as other Structorium languages:
