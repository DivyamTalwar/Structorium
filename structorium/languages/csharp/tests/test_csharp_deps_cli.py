...
from structorium.languages.csharp.deps.cli import cmd_cycles, cmd_deps
...
def test_cmd_deps_single_file_text_output(capsys, monkeypatch):
    ...
    monkeypatch.setattr(
        "structorium.languages.csharp.deps.cli.rel",
        lambda p: "Services/Greeter.cs",
    )
    monkeypatch.setattr(
        "structorium.languages.csharp.deps.cli.get_coupling_score",
...
def test_cmd_cycles_no_cycles_text(capsys, monkeypatch):
    ...
    monkeypatch.setattr(
        "structorium.languages.csharp.deps.cli.detect_cycles",
        lambda graph: ([], 0),
    )
