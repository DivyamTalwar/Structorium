...
import structorium.languages.typescript.phases as phases
from structorium.engine.detectors.coupling import CouplingEdgeCounts
...
def test_phase_structural_uses_lang_thresholds(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(
        "structorium.engine.detectors.large.detect_large_files", _fake_detect_large
    )
    monkeypatch.setattr(
        "structorium.engine.detectors.complexity.detect_complexity", _fake_detect_complexity
    )
    monkeypatch.setattr(
        "structorium.engine.detectors.gods.detect_gods", lambda *a, **k: ([], 0)
    )
    monkeypatch.setattr(
        "structorium.engine.detectors.flat_dirs.detect_flat_dirs", lambda *a, **k: ([], 0)
    )
    monkeypatch.setattr(
        "structorium.languages.typescript.extractors.extract_ts_components", lambda _p: []
    )
    monkeypatch.setattr(
        "structorium.languages.typescript.extractors.detect_passthrough_components",
        lambda _p: [],
    )
    monkeypatch.setattr(
        "structorium.languages.typescript.detectors.concerns.detect_mixed_concerns",
        lambda _p: ([], 0),
    )
    monkeypatch.setattr(
        "structorium.languages.typescript.detectors.props.detect_prop_interface_bloat",
...
def test_phase_coupling_passes_orphaned_options(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(
        "structorium.languages.typescript.detectors.deps.build_dep_graph",
        lambda _path: {},
    )
    monkeypatch.setattr(
        "structorium.engine.detectors.single_use.detect_single_use_abstractions",
        lambda _path, _graph, barrel_names: ([], 0),
    )
    monkeypatch.setattr(
        "structorium.engine.detectors.coupling.detect_coupling_violations",
    ...
    monkeypatch.setattr(
        "structorium.engine.detectors.coupling.detect_cross_tool_imports",
    ...
    monkeypatch.setattr(
        "structorium.languages.typescript.phases._make_boundary_findings",
        lambda single_entries, path, graph, lang, shared_prefix, tools_prefix: ([], 0),
    )
    monkeypatch.setattr(
        "structorium.engine.detectors.graph.detect_cycles",
        lambda _graph: ([], 0),
    )
    ...
    monkeypatch.setattr(
        "structorium.engine.detectors.orphaned.detect_orphaned_files",
        _fake_detect_orphaned_files,
    )
    monkeypatch.setattr(
        "structorium.languages.typescript.detectors.facade.detect_reexport_facades",
        lambda _graph: ([], 0),
    )
    monkeypatch.setattr(
        "structorium.languages.typescript.detectors.patterns.detect_pattern_anomalies_result",
        lambda _path: SimpleNamespace(entries=[], population_size=0),
    )
    monkeypatch.setattr(
        "structorium.engine.detectors.naming.detect_naming_inconsistencies",
