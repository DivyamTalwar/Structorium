...
def load_all() -> None:
    ...
    # Discover user plugins from <active-project-root>/.structorium/plugins/*.py
    try:
        from structorium.core._internal.text_utils import get_project_root

        user_plugin_dir = get_project_root() / ".structorium" / "plugins"
        if user_plugin_dir.is_dir():
            for f in sorted(user_plugin_dir.glob("*.py")):
                spec = importlib.util.spec_from_file_location(
                    f"structorium_user_plugin_{f.stem}", f
                )
