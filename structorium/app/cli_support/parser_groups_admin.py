...
from structorium.languages import get_lang
...
examples:
  ...
  structorium review --external-submit --session-id <id> --import findings.json
  structorium review --merge --similarity 0.8""",
...
Run with no subcommand to generate a full prioritized markdown plan.""",
        epilog="""\
typical workflow:
  structorium scan                       # detect findings
  structorium plan                       # full prioritized markdown
  structorium plan queue                 # compact table of all items
  structorium plan cluster create ...    # group related findings
  structorium plan focus <cluster>       # narrow scope
  structorium next                       # work on the next item
  structorium plan done <id> --attest .. # mark as fixed
...
subcommands:
  ...
  cluster    Manage finding clusters""",
    ...
    p_move = plan_sub.add_parser(
        ...
        epilog="""\
examples:
  structorium plan move security top                    # prioritize a detector
  structorium plan move "unused::src/foo.ts::*" top     # prioritize by pattern
  structorium plan move smells bottom                   # deprioritize smells
  structorium plan move unused before security          # move before another""",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    p_move.add_argument(
    ...
    p_done = plan_sub.add_parser(
        ...
        epilog="""\
examples:
  structorium plan done "unused::src/foo.tsx::React" \\
    --attest "I have actually removed the import and I am not gaming the score."
  structorium plan done security --note "patched XSS" \\
...
def _add_update_skill_parser(sub) -> None:
    p = sub.add_parser(
        "update-skill",
        help="Install or update the structorium skill/agent document",
    )
    p.add_argument(
        "interface",
        nargs="?",
