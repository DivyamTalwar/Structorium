...
from structorium.app.cli_support.parser_groups import (
    _add_config_parser,
    _add_detect_parser,
    _add_dev_parser,
    _add_exclude_parser,
...
examples:
  ...
  structorium review --run-batches --parallel --scan-after-import
"""
...
def _cli_version_string() -> str:
    ...
    try:
        return f"structorium {get_version('structorium')}"
    except PackageNotFoundError:
        return "structorium (version unknown)"
...
def create_parser(*, langs: list[str], detector_names: list[str]) -> argparse.ArgumentParser:
    ...
    parser = _NoAbbrevArgumentParser(
        prog="structorium",
        description="Structorium — Architecture. Automatically Enforced",
