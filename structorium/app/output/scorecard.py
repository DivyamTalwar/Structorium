...
from structorium.app.output.scorecard_parts.theme import (
    ACCENT,
    BG,
    BG_ROW_ALT,
    BG_SCORE,
...
from structorium.state import get_overall_score, get_strict_score
...
def generate_scorecard(state: dict, output_path: str | Path) -> Path:
    ...
    scorecard_draw_mod = importlib.import_module("structorium.app.output.scorecard_parts.draw")  # deferred: depends on PIL
...
def get_badge_config(args, config: dict | None = None) -> tuple[Path | None, bool]:
    ...
    if not disabled:
        disabled = os.environ.get("STRUCTORIUM_NO_BADGE", "").lower() in (
            "1",
            "true",
            "yes",
        )
    ...
    path_str = (
        ...
        or os.environ.get("STRUCTORIUM_BADGE_PATH", "scorecard.png")
    )
    path = Path(path_str)
