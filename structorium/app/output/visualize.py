...
from structorium.app.output.tree_text import render_tree_lines
from structorium.core.file_paths import resolve_scan_file
from structorium.core.fallbacks import (
    log_best_effort_failure,
    print_write_error,
    warn_best_effort,
)
from structorium.core.output_contract import OutputResult
from structorium.core.discovery_api import rel, safe_write_text
...
def _resolve_visualization_lang(path: Path, lang):
    ...
    if lang:
        return lang
    from structorium.languages import auto_detect_lang, get_lang
...
def _fallback_source_files(path: Path) -> list[str]:
    ...
    from structorium.core.discovery_api import find_source_files
...
def cmd_viz(args: argparse.Namespace) -> None:
    ...
    output = Path(getattr(args, "output", None) or ".structorium/treemap.html")
