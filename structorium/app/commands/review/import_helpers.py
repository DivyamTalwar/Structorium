...
from structorium.intelligence.review.importing.contracts import (
...
from structorium.state import coerce_assessment_score
...
def _default_blind_packet_path() -> Path:
    return get_project_root() / ".structorium" / "review_packet_blind.json"
...
def _print_import_error_hints(
    ...
    *,
    ...
    import_cmd = (
        "structorium review --import "
        f"{quoted_import} --attested-external --attest "
        f"\"{_ATTESTED_EXTERNAL_ATTEST_EXAMPLE}\""
    )
    validate_cmd = (
        "structorium review --validate-import "
        f"{quoted_import} --attested-external --attest "
        f"\"{_ATTESTED_EXTERNAL_ATTEST_EXAMPLE}\""
    )
    findings_only_cmd = f"structorium review --import {quoted_import}"
...
def print_assessment_policy_notice(
    ...
    *,
    ...
    if mode == "findings_only":
        ...
        print(
            colorize_fn(
                "  Happy path: use `structorium review --run-batches --parallel --scan-after-import`.",
                "dim",
            )
        )
        print(
            colorize_fn(
                ...
                f"`structorium review --import {import_file} --manual-override --attest \"<why this is justified>\"`.",
                "dim",
            )
        )
        print(
            colorize_fn(
                ...
                f"`structorium review --import {import_file} --attested-external "
...
def print_open_review_summary(state: dict[str, Any], *, colorize_fn) -> str:
    ...
    print(colorize_fn("  Run `structorium show review --status open` to see the work queue", "dim"))
    return "structorium show review --status open"
