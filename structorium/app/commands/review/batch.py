...
from structorium.app.commands.helpers.query import write_query_best_effort
from structorium.app.commands.review import batch_core as batch_core_mod
...
from structorium.core._internal.coercions import (
...
from structorium.intelligence import narrative as narrative_mod
from structorium.intelligence import review as review_mod
from structorium.intelligence.review.feedback_contract import (
    max_batch_findings_for_dimension_count,
)

from .ai_context import enrich_holistic_packet_with_ai_context
...
def _review_packet_dir() -> Path:
    ...
    return _runtime_project_root() / ".structorium" / "review_packets"


def _subagent_runs_dir() -> Path:
    if isinstance(SUBAGENT_RUNS_DIR, Path):
        return SUBAGENT_RUNS_DIR
    return _runtime_project_root() / ".structorium" / "subagents" / "runs"


def _blind_packet_path() -> Path:
    return _runtime_project_root() / ".structorium" / "review_packet_blind.json"
...
def _load_or_prepare_packet(
    ...
    *,
    ...
    next_command = "structorium review --run-batches --runner codex --parallel"
    if retrospective:
        next_command += (
            " --retrospective"
