...
from structorium.app.commands.helpers.rendering import print_agent_plan, print_ranked_actions
from structorium.app.commands.helpers.subjective import print_subjective_followup
from structorium.app.commands.scan import (
    scan_reporting_dimensions as reporting_dimensions_mod,
)
from structorium.app.commands.scan.scan_reporting_presentation import dimension_bar
from structorium.app.commands.status_parts.summary import (
...
from structorium.core.registry import dimension_action_type
...
from structorium.core.output_api import colorize, print_table
...
def _status_next_command(narrative: dict) -> str:
    ...
    return actions[0]["command"] if actions else "structorium next --count 20"
...
def show_agent_plan(narrative: dict, *, plan: dict | None = None) -> None:
    ...
    if plan and (plan.get("queue_order") or plan.get("clusters")):
        print_agent_plan(
            [],
            plan=plan,
            header="  AGENT PLAN (use `structorium next` to see your next task):",
        )
        return
    ...
    print(
        colorize(
            "  AGENT PLAN (use `structorium next --count 20` to inspect more items):",
            "yellow",
        )
    )
    top = actions[0]
...
def _render_area_workflow(sorted_areas: list[tuple[str, list]], *, max_areas: int = 15) -> None:
    ...
    print(colorize("\n  Workflow:", "dim"))
    print(colorize("    1. structorium show <area> --status wontfix --top 50", "dim"))
    print(
        colorize(
...
def show_review_summary(state: dict):
    ...
    print(colorize(f"  Review: {', '.join(parts)} — `structorium show review --status open`", "cyan"))
