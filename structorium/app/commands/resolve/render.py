...
def _print_wontfix_batch_warning(
    ...
    *,
    ...
    print(
        colorize(
            '    The strict score reflects this. Run `structorium show "*" --status wontfix` to review.',
            "dim",
        )
    )
...
def _print_score_movement(
    *,
    ...
    if (
        ...
        print(colorize("\n  Scores unavailable — run `structorium scan`.", "yellow"))
        return
    ...
    if has_review_findings and abs(overall_delta) < 0.05:
        ...
        print(
            colorize(
                "  Run `structorium review --prepare` to get updated assessment scores.",
                "dim",
            )
        )
    elif status == "fixed":
    ...
    *,
    ...
    print(
        colorize(
            ...
            + f"`structorium review --prepare --dimensions {','.join(stale_dims)}`",
            "dim",
        )
    )
...
def _print_next_command(state: dict) -> str:
    ...
    if remaining > 0:
        ...
        print(
            colorize(
                f"\n  {remaining} finding{suffix} remaining — run `structorium next`",
                "dim",
            )
        )
        next_command = "structorium next"
