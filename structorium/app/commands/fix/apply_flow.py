...
from structorium.languages._framework.base.types import FixResult
...
def _apply_and_report(
    ...
    if typecheck_cmd:
        next_action = (
            f"Run `{typecheck_cmd}` to verify, then `structorium scan` to update state"
        )
    else:
        next_action = "Run `structorium scan` to update state"
    write_query(
        {
            "command": "fix",
            "fixer": fixer_name,
        ...
        for sym in r["removed"]:
            ...
            if fid in state["findings"] and state["findings"][fid]["status"] == "open":
                ...
                state["findings"][fid]["note"] = (
                    f"auto-fixed by structorium fix {fixer_name}"
                )
...
def _print_fix_retro(
    ...
    checklist += [
        "Run `structorium scan` to update state. Did score improve as expected?",
