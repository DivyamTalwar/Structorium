...
def _cmd_cluster_show(args: argparse.Namespace) -> None:
    ...
    print(colorize(f'    Resolve all:  structorium plan done "{cluster_name}" --note "<what>" --attest "..."', "dim"))
    print(colorize(f"    Drill in:     structorium next --cluster {cluster_name} --count 10", "dim"))
    print(colorize(f"    Skip:         structorium plan skip {cluster_name}", "dim"))
