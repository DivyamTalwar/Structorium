...
from structorium import scoring as scoring_mod
from structorium.app.commands.scan.scan_reporting_subjective import (
    build_subjective_followup,
)
from structorium.engine.planning.scorecard_projection import (
    scorecard_subjective_entries,
)
from structorium.engine.work_queue import ATTEST_EXAMPLE, group_queue_items
from structorium.intelligence.integrity import (
...
from structorium.core.paths_api import read_code_snippet
...
def _render_tier_navigator(queue: dict) -> None:
    ...
    print(
        colorize(
            "    Switch: `structorium next --tier 1` | `structorium next --tier 2` | "
            "`structorium next --tier 3` | `structorium next --tier 4`",
            "dim",
        )
    )
...
def is_auto_fix_command(command: str | None) -> bool:
    ...
    return cmd.startswith("structorium fix ") and "--dry-run" in cmd
...
def _render_cluster_item(item: dict) -> None:
    ...
    # Resolution hints
    print(colorize(f'\n  Resolve all:   structorium plan done "{cluster_name}" '
                   f'--note "<what>" --attest "{ATTEST_EXAMPLE}"', "dim"))
    print(colorize(f"  Drill in:      structorium next --cluster {cluster_name} --count 10",
                   "dim"))
    print(colorize(f"  Skip cluster:  structorium plan skip {cluster_name}", "dim"))
...
def _render_item(
    ...
    if kind == "subjective_dimension":
        ...
        print(
            colorize(
                f"  Action: {item.get('primary_command', 'structorium review --prepare')}",
                "cyan",
            )
        )
        print(colorize(
    ...
    if potentials and detector and dim_scores:
        try:
            from structorium.scoring import compute_score_impact
        ...
        try:
            from structorium.scoring import compute_health_breakdown
...
def show_empty_queue(queue: dict, tier: int | None, strict: float | None) -> bool:
    ...
    if tier is not None:
        ...
        if available:
            commands = " | ".join(
                f"structorium next --tier {tier_num}" for tier_num in available
            )
...
def render_single_item_resolution_hint(items: list[dict]) -> None:
    ...
    if detector_name == "subjective_review":
        print(colorize("\n  Review with:", "dim"))
        primary = item.get(
            "primary_command", "structorium show subjective"
        )
        print(f"    {primary}")
        if is_holistic_subjective_finding(item):
            print("    structorium review --prepare")
        return
    ...
    print(
        f'    structorium plan done "{item["id"]}" --note "<what you did>" '
        f'--attest "{ATTEST_EXAMPLE}"'
    )
    print(
        f'    structorium plan skip --permanent "{item["id"]}" --note "<why>" '
        f'--attest "{ATTEST_EXAMPLE}"'
    )
...
def render_followup_nudges(
    ...
    *,
    ...
    if parts:
        ...
        print(colorize("  Run `structorium show subjective` for details.", "dim"))
