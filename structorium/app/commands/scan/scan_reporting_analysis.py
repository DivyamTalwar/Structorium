...
from structorium.engine.plan import has_living_plan, load_plan
    ...
    *,
    ...
    if chronic_count > 0:
        warnings.append(
            ...
            "run `structorium show --chronic` to see them."
        )
    ...
    print(colorize("  Run `structorium next` for the highest-priority item.", "dim"))
    if _has_plan:
        print(colorize("  Run `structorium plan` to see the updated living plan.", "dim"))
    print(colorize("  Run `structorium status` for the full dashboard.", "dim"))
    print()
