...
from structorium.app.commands.scan.scan_reporting_text import build_workflow_guide
from structorium.engine.planning import scorecard_projection as scorecard_projection_mod
from structorium.core import registry as registry_mod
from structorium.core._internal.text_utils import PROJECT_ROOT
from structorium.core import skill_docs as skill_docs_mod
from structorium.engine.work_queue import ATTEST_EXAMPLE
...
def _is_agent_environment() -> bool:
    return bool(
        ...
        or os.environ.get("STRUCTORIUM_AGENT")
...
def _try_auto_update_skill() -> None:
    ...
    try:
        from structorium.app.commands.update_skill import (
            resolve_interface,
...
def _print_llm_summary(
    ...
    Only shown when running inside an agent (CLAUDE_CODE or STRUCTORIUM_AGENT env).
    """
    ...
    try:
        from structorium.engine.plan import load_plan
        _plan = load_plan()
        _has_plan = bool(
    ...
    if _has_plan:
        ...
        print("Use `structorium next` which respects the plan.")
        print("Use `structorium plan` to view and update it.\n")
    ...
    if _has_plan:
        print("\nFollow the living plan: `structorium next` for your next task,")
        print("`structorium plan` to view the full queue.")
    else:
