...
def _load_scorecard_helpers():
    ...
    """
    try:
        scorecard_module = importlib.import_module("structorium.app.output.scorecard")
    except ImportError:
        return None, None
...
def emit_scorecard_badge(
    ...
    if not callable(generate_scorecard) or not callable(get_badge_config):
        explicit_badge_request = bool(
            ...
            or os.environ.get("STRUCTORIUM_BADGE_PATH")
        )
        if explicit_badge_request:
            print(
                colorize(
                    "  Scorecard support not installed. Install with: pip install \"structorium[scorecard]\"",
                    "yellow",
                )
            )
