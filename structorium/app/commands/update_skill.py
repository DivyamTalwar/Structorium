"""update-skill command: install or update the structorium skill document."""
...
from structorium.core.skill_docs import (
    SKILL_BEGIN,
    SKILL_END,
    SKILL_TARGETS,
    SKILL_VERSION,
...
_RAW_BASE = (
    "https://raw.githubusercontent.com/peteromallet/structorium/main/docs"
)


def _download(filename: str) -> str:
    """Download a file from the structorium docs directory on GitHub."""
...
def _replace_section(file_content: str, new_section: str) -> str:
    """Replace the structorium section in a shared file, preserving surrounding content.

    Uses first ``<!-- structorium-begin -->`` and last ``<!-- structorium-end -->``
...
def update_installed_skill(interface: str) -> bool:
    ...
    if "structorium-skill-version" not in skill_content:
...
def cmd_update_skill(args: argparse.Namespace) -> None:
    """Install or update the structorium skill document."""
    ...
    if not interface:
        ...
        print(f"Install with: structorium update-skill <{names}>")
        return
