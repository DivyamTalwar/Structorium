...
from structorium.app.commands.helpers.lang import auto_detect_lang_name
...
def _sole_existing_lang_state_file() -> Path | None:
    ...
    state_dir = PROJECT_ROOT / ".structorium"
...
def state_path(args) -> Path | None:
    ...
    if lang_name:
        resolved = PROJECT_ROOT / ".structorium" / f"state-{lang_name}.json"
...
def require_completed_scan(state: dict) -> bool:
    ...
    if not has_completed_scan:
        print(colorize("No scans yet. Run: structorium scan", "yellow"))
