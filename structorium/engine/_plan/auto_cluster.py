...
from structorium.core.registry import DETECTORS, DetectorMeta
from structorium.engine._plan.schema import Cluster, PlanModel, ensure_plan_defaults
from structorium.engine._plan.stale_dimensions import (
    SUBJECTIVE_PREFIX,
    _current_unscored_ids,
)
from structorium.engine._state.schema import StateModel, utc_now
...
_ACTION_TYPE_TEMPLATES: dict[str, str] = {
    "reorganize": "reorganize with structorium move",
...
def _generate_action(
    ...
    if subtype and meta.fixers:
        ...
        if matched_fixer:
            return f"structorium fix {matched_fixer} --dry-run"
    elif meta.action_type == "auto_fix" and meta.fixers and not meta.needs_judgment:
        # Pure auto-fix detector, no subtypes
        return f"structorium fix {meta.fixers[0]} --dry-run"

    if meta.tool == "move":
        return "structorium move"
...
def auto_cluster_findings(plan: PlanModel, state: StateModel) -> int:
    ...
    if len(unscored_queue_ids) >= _MIN_UNSCORED_CLUSTER_SIZE:
        ...
        action = f"structorium review --prepare --dimensions {','.join(cli_keys)}"
