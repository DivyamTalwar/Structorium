...
from structorium.engine.planning.common import TIER_LABELS
from structorium.engine.planning.types import PlanState
from structorium.engine.work_queue import QueueBuildOptions, build_work_queue
from structorium.scoring import DIMENSIONS, DISPLAY_NAMES
from structorium.state import get_objective_score, get_overall_score, get_strict_score
from structorium.core.output_api import LOC_COMPACT_THRESHOLD
...
def _plan_header(state: PlanState, stats: dict) -> list[str]:
    ...
    lines = [
        f"# Structorium Plan — {date.today().isoformat()}",
        "",
        f"{header_score} | "
...
def _plan_dimension_table(state: PlanState) -> list[str]:
    ...
    from structorium.engine.planning.dimension_rows import scorecard_dimension_rows
