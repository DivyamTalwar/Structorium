...
from structorium.core.enums import finding_status_tokens
...
from structorium.scoring import DISPLAY_NAMES
...
def primary_command_for_finding(
    ...
    if meta and meta.action_type == "auto_fix" and meta.fixers:
        ...
        if available_fixers:
            return f"structorium fix {available_fixers[0]} --dry-run"
    if detector == "subjective_review":
        if is_holistic_subjective_finding(item):
            return "structorium review --prepare"
        return "structorium show subjective"
    return f'structorium plan done "{item.get("id", "")}" --note "<what you did>" --attest "{ATTEST_EXAMPLE}"'
...
def build_subjective_items(
    ...
    for entry in subjective_entries:
        ...
        if is_unassessed:
            primary_command = "structorium review --prepare"
        elif is_stale:
            if cli_keys:
                primary_command = (
                    "structorium review --prepare --dimensions " + ",".join(cli_keys)
                )
            else:
                primary_command = "structorium review --prepare"
        else:
            if open_review > 0:
                primary_command = "structorium show review --status open"
            elif cli_keys:
                primary_command = (
                    "structorium review --prepare --dimensions " + ",".join(cli_keys)
                )
