...
from structorium.engine._work_queue.helpers import (
    ALL_STATUSES,
    ATTEST_EXAMPLE,
    build_subjective_items,
    scope_matches,
)
from structorium.engine._work_queue.ranking import (
    build_finding_items,
...
def _collapse_clusters(items: list[dict], plan: dict) -> list[dict]:
    ...
    for cname, members in cluster_members.items():
        ...
        if "structorium fix" in action:
            action_type = "auto_fix"
        elif "structorium move" in action:
            ...
            if action_type == "auto_fix" and "structorium fix" not in action:
        ...
        if not primary_command:
            primary_command = f"structorium next --cluster {cname} --count 10"
