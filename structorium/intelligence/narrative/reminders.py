...
from structorium.core.fallbacks import log_best_effort_failure
from structorium.intelligence.narrative._constants import (
    _FEEDBACK_URL,
    _REMINDER_DECAY_THRESHOLD,
    STRUCTURAL_MERGE,
)
from structorium.state import StateModel, get_strict_score, path_scoped_findings
...
def _auto_fixer_reminder(actions: list[dict]) -> list[dict]:
    ...
    if total <= 0:
        return []
    first_cmd = auto_fix_actions[0].get("command", "structorium fix <fixer> --dry-run")
    return [
        {
        ...
        reminders.append(
            {
                ...
                "message": "Wontfix debt is growing. Review stale decisions: `structorium show --status wontfix`.",
                "command": "structorium show --status wontfix",
            }
        )
        ...
        reminders.append(
            {
                ...
                "message": (
                    ...
                    f'`structorium show "*" --status wontfix`'
                ),
                "command": 'structorium show "*" --status wontfix',
            }
        )
    ...
    for dim in dimensions.get("stagnant_dimensions", []):
        ...
        if strict >= 99:
            ...
            message = (
                ...
                f"Try tackling it from a different angle — run `structorium next` to find the right entry point."
            )
        reminders.append(
            {
                "type": "stagnant_nudge",
    ...
    return [
        {
            ...
            "message": (
                ...
                f"({', '.join(parts)}). "
                f"Override with `structorium zone set <file> production` "
                f"if any are misclassified."
            ),
            "command": "structorium zone show",
        }
    ]
            ...
            reminders.append(
                {
                    ...
                    f"Run `structorium show review --status open` to see the work queue.",
                    "command": "structorium show review --status open",
                }
            )
        ...
        reminders.append(
            {
                ...
                "`structorium review --prepare` to refresh, or reset with "
                "`structorium scan --reset-subjective` before a clean rerun.",
                "command": "structorium review --prepare",
            }
        )
            ...
            reminders.append(
                {
                    ...
                    "message": (
                        ...
                        "to catch issues linters miss: structorium review --prepare"
                    ),
                    "command": "structorium review --prepare",
                }
            )
    ...
    return [
        {
            ...
            "message": (
                ...
                f"`structorium review --prepare --dimensions {dims_arg}`"
            ),
            "command": f"structorium review --prepare --dimensions {dims_arg}",
        }
    ]
...
def _review_staleness_reminder(state: StateModel, config: dict | None) -> list[dict]:
    ...
    return [
        {
            "type": "review_stale",
            "message": f"Design review is {age_days} days old — run: structorium review --prepare",
            "command": "structorium review --prepare",
        }
    ]
...
def _feedback_reminder(
    ...
    if high_fp_dets:
        ...
        nudge_msg = (
            f"Score has plateaued — if you suspect structorium is missing patterns "
        ...
        nudge_msg = (
            f"If you notice patterns structorium doesn't detect, false positives, "
