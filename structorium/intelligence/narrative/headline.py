...
def _compute_headline(
    ...
    if review_count > 0:
        ...
        if uninvestigated > 0:
            review_suffix = (
                f" ({review_count} review finding{s} \u2014 run `structorium show review --status open`)"
            )
        else:
    ...
    if phase == "middle_grind":
        ...
        if lowest:
            ...
            return (
                ...
                f"needs attention — run `structorium next` to start."
            )
        if open_count > 0:
            return f"{open_count} findings open. Run `structorium next` for the highest-priority item."
