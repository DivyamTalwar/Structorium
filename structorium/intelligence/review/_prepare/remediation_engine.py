...
from structorium.scoring import CONFIDENCE_WEIGHTS, HOLISTIC_MULTIPLIER
...
def empty_plan(state: dict[str, Any], lang_name: str) -> str:
    ...
    return (
        ...
        "```bash\n"
        f"structorium --lang {lang_name} review --prepare --path <src>\n"
        "```\n"
    )
...
def _render_usage(lines: list[str], lang_name: str) -> None:
    ...
    append("4. Run `structorium scan` to update automated findings and score")
...
def _render_entry(
    ...
    append("### After fixing\n")
    append("```bash")
    append(f'structorium --lang {lang_name} resolve fixed "{entry["id"]}"')
    append("```\n")
    append("---\n")
...
def _render_re_evaluate(lines: list[str], lang_name: str) -> None:
    ...
    append("```bash")
    append(f"structorium --lang {lang_name} review --prepare --path <src>")
    append("# Agent investigates batches and writes findings.json")
    append(f"structorium --lang {lang_name} review --import findings.json")
    append("```\n")
    append(
