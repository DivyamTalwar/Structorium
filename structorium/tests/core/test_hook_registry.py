...
import structorium.hook_registry as registry_mod
from structorium.hook_registry import clear_lang_hooks_for_tests, get_lang_hook
...
def test_get_lang_hook_reloads_after_test_clear() -> None:
    ...
    importlib.import_module("structorium.languages.python")
...
def test_get_lang_hook_retries_after_import_failure(monkeypatch) -> None:
    ...
    def _fake_import_module(name: str, package: str | None = None):
        if name == "structorium.languages.retrylang":
            attempts["count"] += 1
            if attempts["count"] == 1:
