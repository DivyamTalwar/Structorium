...
_PACKAGE_ROOT = _PROJECT_ROOT / "structorium"
_ALLOWED_COMPAT_MODULES = {
    "structorium/utils.py",
    "structorium/file_discovery.py",
}
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                if alias.name in {"structorium.utils", "structorium.file_discovery"}:
                    violations.append(f"{rel}:{node.lineno} import {alias.name}")
        elif isinstance(node, ast.ImportFrom):
            module = node.module or ""
            if module in {"structorium.utils", "structorium.file_discovery"}:
                violations.append(f"{rel}:{node.lineno} from {module} import ...")
                continue
            if module == "structorium":
                for alias in node.names:
                    if alias.name in {"utils", "file_discovery"}:
                        violations.append(
                            f"{rel}:{node.lineno} from structorium import {alias.name}"
                        )
    return violations
