"""Tests for structorium.scorecard — helper functions (no PIL/image generation)."""
...
from structorium.app.output.scorecard import (
    SCALE,
...
from structorium.app.output.scorecard_parts.meta import (
...
class TestGetBadgeConfig:
    ...
    def test_env_var_disables(self, monkeypatch):
        monkeypatch.setenv("STRUCTORIUM_NO_BADGE", "true")
        args = SimpleNamespace()
        path, disabled = get_badge_config(args)
        assert disabled is True
        assert path is None
    ...
    def test_env_var_disable_with_1(self, monkeypatch):
        monkeypatch.setenv("STRUCTORIUM_NO_BADGE", "1")
        args = SimpleNamespace()
        _, disabled = get_badge_config(args)
        assert disabled is True

    def test_env_var_disable_with_yes(self, monkeypatch):
        monkeypatch.setenv("STRUCTORIUM_NO_BADGE", "yes")
        args = SimpleNamespace()
        _, disabled = get_badge_config(args)
        assert disabled is True

    def test_env_var_badge_path(self, monkeypatch):
        monkeypatch.setenv("STRUCTORIUM_BADGE_PATH", "/custom/env/badge.png")
        monkeypatch.delenv("STRUCTORIUM_NO_BADGE", raising=False)
        args = SimpleNamespace()
        path, disabled = get_badge_config(args)
        assert disabled is False
    ...
    def test_cli_badge_path_overrides_env(self, monkeypatch):
        monkeypatch.setenv("STRUCTORIUM_BADGE_PATH", "/env/path.png")
        monkeypatch.delenv("STRUCTORIUM_NO_BADGE", raising=False)
    ...
    def test_no_badge_flag_takes_precedence_over_env_path(self, monkeypatch):
        monkeypatch.setenv("STRUCTORIUM_BADGE_PATH", "/some/path.png")
        monkeypatch.delenv("STRUCTORIUM_NO_BADGE", raising=False)
        args = SimpleNamespace(no_badge=True)
        path, disabled = get_badge_config(args)
        assert disabled is True
    ...
    def test_unset_env_var_does_not_disable(self, monkeypatch):
        monkeypatch.delenv("STRUCTORIUM_NO_BADGE", raising=False)
        monkeypatch.delenv("STRUCTORIUM_BADGE_PATH", raising=False)
        args = SimpleNamespace()
        path, disabled = get_badge_config(args)
        assert disabled is False
        assert path is not None
...
class TestGetProjectName:
    def test_gh_cli_success(self, monkeypatch):
        import structorium.app.output.scorecard_parts.meta as meta
