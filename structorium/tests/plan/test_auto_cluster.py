...
from structorium.engine._plan.auto_cluster import (
    auto_cluster_findings,
    _cluster_name_from_key,
    _grouping_key,
)
from structorium.engine._plan.operations import (
    create_cluster,
    add_to_cluster,
    remove_from_cluster,
)
from structorium.engine._plan.schema import empty_plan, ensure_plan_defaults
from structorium.engine._work_queue.core import (
    QueueBuildOptions,
    build_work_queue,
    _collapse_clusters,
)
from structorium.engine._work_queue.ranking import item_sort_key
...
def test_grouping_key_auto_fix():
    from structorium.core.registry import DETECTORS
    f = _finding("a", "unused")
...
def test_collapse_clusters_replaces_members():
    ...
    plan["clusters"]["auto/unused"] = {
        ...
        "action": "structorium fix unused-imports --dry-run",
        "user_modified": False,
    }
...
def test_generate_action_always_returns_something():
    ...
    from structorium.core.registry import DETECTORS
    from structorium.engine._plan.auto_cluster import _generate_action
...
def test_generate_action_strips_subtype_examples():
    ...
    from structorium.engine._plan.auto_cluster import _strip_guidance_examples
...
def test_collapse_fallback_action():
    ...
    assert "structorium next --cluster" in cluster_items[0]["primary_command"]
...
def test_narrative_actions_mention_clusters():
    ...
    from structorium.intelligence.narrative.action_engine import _annotate_with_clusters

    actions = [
        {"detector": "unused", "count": 5, "command": "structorium fix unused-imports --dry-run",
    ...
    assert actions[0]["command"] == "structorium next"
...
def test_initial_review_cluster_created():
    ...
    assert "structorium review --prepare --dimensions" in cluster["action"]
