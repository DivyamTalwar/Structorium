"""Tests for structorium.engine.detectors.coupling — coupling analysis detectors."""
...
from structorium.engine.detectors.coupling import (
...
class TestDetectCouplingViolations:
    ...
    def test_finds_shared_to_tools_imports(self):
        ...
        with patch(
            "structorium.engine.detectors.coupling.rel",
    ...
    def test_accepts_relative_graph_keys_with_absolute_prefixes(self):
        ...
        with patch("structorium.engine.detectors.coupling.rel", side_effect=lambda p: p):
            entries, total_edges = detect_coupling_violations(
                Path("/project"), graph, SHARED_PREFIX, TOOLS_PREFIX
            )
        assert len(entries) == 1
