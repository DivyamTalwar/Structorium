...
# ── Knip adapter ────────────────────────────────────────────────────────────
from structorium.languages.typescript.detectors.knip_adapter import detect_with_knip
...
from structorium.languages.python.detectors.ruff_smells import detect_with_ruff_smells  # noqa: E402,I001
...
from structorium.languages.python.detectors.bandit_adapter import (  # noqa: E402
    _to_security_entry,
    detect_with_bandit,
)
...
class TestBanditExcludeIntegration:
    ...
    def test_detect_lang_security_passes_exclusions_to_bandit(self):
        from structorium.languages.python import PythonConfig

        config = PythonConfig()
        captured_kwargs = {}

        def _fake_bandit(path, zone_map, **kwargs):
            from structorium.languages.python.detectors.bandit_adapter import (
                BanditRunStatus,
                BanditScanResult,
            )
        ...
        with patch(
            "structorium.languages.python.detect_with_bandit", _fake_bandit
        ), patch(
            "structorium.languages.python.collect_exclude_dirs",
...
from structorium.engine.detectors.jscpd_adapter import (  # noqa: E402
    _parse_jscpd_report,
    detect_with_jscpd,
)
...
class TestJscpdAdapter:
    ...
    def test_skips_artifact_paths(self, tmp_path):
        f_artifact = str(tmp_path / ".structorium" / "cache.py")
        f_src = str(tmp_path / "pkg" / "module.py")
        fragment = "x = 1\ny = 2\nz = 3\nw = 4"
        report = {
            "duplicates": [
    ...
    def test_detect_command_includes_artifact_ignores(self, tmp_path):
        ...
        def _fake_run(cmd, **kwargs):
            ...
            assert "**/.structorium/**" in ignore_value
        ...
        with patch("subprocess.run", side_effect=_fake_run), patch(
            "structorium.engine.detectors.jscpd_adapter.collect_exclude_dirs",
            return_value=fake_dirs,
        ), patch(
            "structorium.engine.detectors.jscpd_adapter.get_exclusions",
            return_value=(),
        ), patch(
...
from structorium.languages.python.detectors.import_linter_adapter import (  # noqa: E402
...
from structorium.core.source_discovery import collect_exclude_dirs  # noqa: E402


class TestCollectExcludeDirs:
    def test_returns_absolute_paths(self, tmp_path):
        with patch(
            "structorium.core.source_discovery.get_exclusions", return_value=()
        ):
    ...
    def test_includes_runtime_exclusions(self, tmp_path):
        with patch(
            "structorium.core.source_discovery.get_exclusions",
...
from structorium.languages.python.detectors.unused import detect_unused  # noqa: E402
...
class TestRuffSmellsExcludeFlag:
    ...
    def test_ruff_smells_passes_exclude_flag(self, tmp_path):
        ...
        with patch("subprocess.run", side_effect=_capture_run), patch(
            "structorium.languages.python.detectors.ruff_smells._collect_exclude_dirs",
            return_value=fake_dirs,
        ):
...
class TestRuffUnusedExcludeFlag:
    ...
    def test_ruff_unused_passes_exclude_flag(self, tmp_path):
        ...
        with patch("subprocess.run", side_effect=_capture_run), patch(
            "structorium.languages.python.detectors.unused._collect_exclude_dirs",
            return_value=fake_dirs,
        ), patch(
            "structorium.languages.python.detectors.unused.find_py_files",
            return_value=[],
        ):
