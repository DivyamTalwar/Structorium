...
import structorium.app.commands.scan.scan_reporting_analysis as scan_reporting_analysis_mod
import structorium.app.commands.scan.scan_reporting_dimensions as scan_reporting_dimensions_mod
import structorium.app.commands.scan.scan_reporting_llm as scan_reporting_llm_mod
import structorium.app.commands.scan.scan_reporting_summary as scan_reporting_summary_mod
...
def test_show_score_delta_handles_unavailable_scores(monkeypatch, capsys):
    import structorium.state as state_mod
...
def test_show_post_scan_analysis_surfaces_warnings_and_headline(monkeypatch, capsys):
    import structorium.intelligence.narrative as narrative_mod
    ...
    assert "structorium next" in out
    assert "structorium status" in out
...
def test_print_llm_summary_respects_env_and_includes_dimension_table(
    ...
    import structorium.core.registry as registry_mod
    import structorium.scoring as scoring_mod
    import structorium.state as state_mod

    monkeypatch.setenv("STRUCTORIUM_AGENT", "1")
    ...
    scan_reporting_llm_mod._print_llm_summary(
        ...
        narrative={
            ...
            "actions": [
                {"command": "structorium next", "description": "Resolve top finding"}
            ],
        },
    ...
    assert "Top action: `structorium next` — Resolve top finding" in out
...
def test_show_scorecard_dimensions_and_dimension_hints(monkeypatch, capsys):
    ...
    assert "run `structorium show structural`" in hint_out
    assert "run `structorium review --run-batches --runner codex --parallel --scan-after-import`" in hint_out
