...
from structorium.app.commands.helpers.rendering import (
    print_agent_plan,
    print_ranked_actions,
    print_replacement_groups,
)
from structorium.app.commands.helpers.state import require_completed_scan, state_path
...
def test_print_agent_plan_next_command(capsys):
    ...
    print_agent_plan(["step"], next_command="structorium next")
    out = capsys.readouterr().out
...
def test_state_path_from_lang_arg():
    """--lang argument produces .structorium/state-{lang}.json."""
...
def _make_followup(
    *,
    ...
    command: str = "structorium review",
...
def test_subjective_followup_low_assessed(capsys):
    ...
    assert "structorium review" in out
