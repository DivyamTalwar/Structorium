"""Tests for structorium.app.commands.dev_cmd."""
...
import structorium.app.commands.dev_cmd as dev_mod
...
def test_scaffold_lang_creates_standard_files(tmp_path, monkeypatch):
    ...
    lang_dir = tmp_path / "structorium" / "languages" / "ruby"
...
def test_scaffold_lang_force_overwrites(tmp_path, monkeypatch):
    ...
    target = tmp_path / "structorium" / "languages" / "ruby" / "commands.py"
...
def test_scaffold_lang_wires_pyproject_once(tmp_path, monkeypatch):
    ...
    assert "structorium.languages.ruby.tests*" not in first
    assert "structorium/languages/ruby/tests" in first

    dev_mod.cmd_dev(_args(force=True, wire_pyproject=True))
    second = pyproject.read_text()
    assert second.count("structorium.languages.ruby.tests*") == 0
    assert second.count("structorium/languages/ruby/tests") == 1
