...
Covers:
- structorium.engine._state.merge (MergeScanOptions, merge_scan)
- structorium.intelligence.review.context_holistic.readers (_abs, _read_file_contents)
- structorium.app.cli_support.parser_groups_admin (parser builders, helpers)
- structorium.app.commands.move.move_apply (rollback, apply helpers)
- structorium.languages._framework.base.shared_phases (entries_to_findings, log, find_external)
"""
...
import structorium.app.cli_support.parser_groups_admin as parser_admin_mod

# ── Module 4: move_apply ──────────────────────────────────────────────
import structorium.app.commands.move.move_apply as move_apply_mod

# ── Module 1: engine._state.merge ─────────────────────────────────────
import structorium.engine._state.merge as merge_mod

# ── Module 2: readers ─────────────────────────────────────────────────
import structorium.intelligence.review.context_holistic.readers as readers_mod

# ── Module 5: shared_phases ───────────────────────────────────────────
import structorium.languages._framework.base.shared_phases as shared_phases_mod
from structorium.engine._state.merge import MergeScanOptions, merge_scan
...
class TestMergeScan:
    ...
    def _make_state(self):
        from structorium.engine._state.schema import empty_state, ensure_state_defaults
...
class TestReaders:
    ...
    @patch("structorium.intelligence.review.context_holistic.readers.resolve_path")
    ...
    @patch("structorium.intelligence.review.context_holistic.readers.read_file_text")
    @patch("structorium.intelligence.review.context_holistic.readers.resolve_path")
...
class TestFixerHelpLines:
    @patch("structorium.app.cli_support.parser_groups_admin.get_lang")
...
class TestFixParser:
    def test_fix_parser_args(self):
        ...
        sub = parser.add_subparsers(dest="command")
        with patch(
            "structorium.app.cli_support.parser_groups_admin.get_lang"
        ) as mock_get_lang:
...
class TestRollbackWrittenFiles:
    @patch("structorium.app.commands.move.move_apply.restore_files_best_effort")
    @patch("structorium.app.commands.move.move_apply.warn_best_effort")
...
class TestRollbackMoveTarget:
    ...
    @patch("structorium.app.commands.move.move_apply.shutil.move", side_effect=OSError("fail"))
    @patch("structorium.app.commands.move.move_apply.warn_best_effort")
...
class TestLogPhaseSummary:
    @patch("structorium.languages._framework.base.shared_phases.log")
...
class TestFindExternalTestFiles:
    def test_finds_test_files_outside_scanned_path(self, tmp_path):
        ...
        with patch(
            "structorium.languages._framework.base.shared_phases.get_project_root",
