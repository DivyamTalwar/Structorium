"""Tests for structorium.cli — argument parsing, state path resolution, helpers."""
...
import structorium.app.commands.helpers.lang as lang_helpers_mod
import structorium.cli as cli_mod
from structorium.app.commands.helpers.lang import resolve_lang, resolve_lang_settings
from structorium.app.commands.helpers.query import write_query
from structorium.app.commands.helpers.runtime_options import (
    LangRuntimeOptionsError,
    resolve_lang_runtime_options,
)
from structorium.app.commands.helpers.score import (
    coerce_target_score,
    target_strict_score_from_config,
)
from structorium.cli import (
...
from structorium.languages.csharp import CSharpConfig
...
class TestCreateParser:
    ...
    def test_top_level_version_flag(self, parser, capsys):
        ...
        assert out.startswith("structorium")
...
class TestStatePath:
    def test_auto_detects_lang_when_no_state_or_lang(self):
        ...
        with patch("structorium.languages.auto_detect_lang", return_value="python"):
        ...
        with patch("structorium.languages.auto_detect_lang", return_value=None):
            result = state_path(args)
    ...
    def test_returns_lang_based_path_when_lang_set(self):
        ...
        assert result is not None
        assert "state-python.json" in str(result)
        assert ".structorium" in str(result)
    ...
    def test_non_scan_falls_back_to_sole_existing_lang_state(
        ...
        state_dir = tmp_path / ".structorium"
        state_dir.mkdir()
        existing = state_dir / "state-typescript.json"
        existing.write_text("{}")
        monkeypatch.setattr("structorium.app.commands.helpers.state.PROJECT_ROOT", tmp_path)
    ...
    def test_non_scan_without_lang_uses_sole_existing_state(
        ...
        monkeypatch.setattr("structorium.app.commands.helpers.state.PROJECT_ROOT", tmp_path)
        monkeypatch.setattr(
            "structorium.app.commands.helpers.state.auto_detect_lang_name",
            lambda _args: None,
        )
...
class TestResolveDefaultPath:
    ...
    def test_review_uses_scan_path_from_state(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "structorium.app.commands.helpers.state.PROJECT_ROOT", project_root
        )

        with (
            patch("structorium.cli.state_path", return_value=tmp_path / "state.json"),
            patch("structorium.cli.load_state", return_value=saved_state),
        ):
    ...
    def test_review_falls_back_to_lang_default_when_no_scan_path(self, monkeypatch):
        ...
        with (
            patch("structorium.cli.state_path", return_value=None),
            patch("structorium.cli.load_state", return_value={}),
            patch("structorium.cli.resolve_lang") as mock_lang,
        ):
    ...
    def test_review_falls_back_when_state_load_raises(self, monkeypatch):
        ...
        with (
            patch("structorium.cli.state_path", return_value=None),
            patch("structorium.cli.load_state", side_effect=OSError("no file")),
            patch("structorium.cli.resolve_lang") as mock_lang,
        ):
            mock_lang.return_value = SimpleNamespace(default_src="src")
            args = SimpleNamespace(command="review", path=None)
    ...
    def test_non_review_command_uses_lang_default(self):
        with patch("structorium.cli.resolve_lang") as mock_lang:
            mock_lang.return_value = SimpleNamespace(default_src="src")
...
class TestResolveLang:
    ...
    def test_auto_detect_uses_path_when_it_looks_like_project_root(
        ...
        monkeypatch.setattr("structorium.utils.PROJECT_ROOT", cwd_root)
    ...
    def test_auto_detect_falls_back_to_project_root_for_subdir_path(
        ...
        monkeypatch.setattr("structorium.utils.PROJECT_ROOT", root)
    ...
    def test_lang_config_markers_include_plugin_markers(self, monkeypatch):
        ...
        monkeypatch.setattr("structorium.languages.available_langs", lambda: ["dummy"])
        monkeypatch.setattr("structorium.languages.get_lang", lambda _name: DummyCfg())
...
class TestWriteQuery:
    def test_writes_valid_json(self, tmp_path, monkeypatch):
        query_file = tmp_path / ".structorium" / "query.json"
        monkeypatch.setattr(
            "structorium.app.commands.helpers.query.query_file_path",
            lambda: query_file,
        )
...
class TestApplyPersistedExclusions:
    def test_cli_exclusions_applied(self, monkeypatch):
        captured = []
        monkeypatch.setattr(
            "structorium.cli.set_exclusions", lambda pats: captured.extend(pats)
        )
    ...
    def test_no_exclusions_does_nothing(self, monkeypatch):
        called = []
        monkeypatch.setattr(
            "structorium.cli.set_exclusions", lambda pats: called.append(pats)
        )
    ...
    def test_smoke_fixture_commands_parse(self):
        ...
        scan_args = parser.parse_args(
            [
                ...
                "scan",
                "--path",
                "structorium/tests/fixtures/cli_smoke_project/src",
                "--state",
                "structorium/tests/snapshots/cli_smoke/state-python.json",
                "--no-badge",
            ]
        )
