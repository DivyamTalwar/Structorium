"""Tests for structorium.visualize — data preparation, tree building, aggregation, esc()."""
...
from structorium.app.commands.helpers.runtime import CommandRuntime
from structorium.app.output._viz_cmd_context import load_cmd_context
from structorium.app.output.tree_text import _aggregate, _print_tree
from structorium.app.output.visualize import (
    D3_CDN_URL,
    _collect_file_data,
    _build_tree,
    cmd_viz,
...
from structorium.core.output_contract import OutputResult
...
class TestCollectFileData:
    ...
    def test_lang_resolution_failures_use_best_effort_fallback(
        ...
        monkeypatch.setattr(
            "structorium.languages.auto_detect_lang",
            lambda _root: "python",
        )
        monkeypatch.setattr(
            "structorium.languages.get_lang",
            lambda _name: (_ for _ in ()).throw(RuntimeError("plugin load failed")),
        )
        monkeypatch.setattr("structorium.languages.available_langs", lambda: ["python"])
...
class TestLoadCmdContext:
    def test_uses_preloaded_state_when_available(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "structorium.app.output._viz_cmd_context.resolve_lang", lambda _a: None
        )
        calls = []
        monkeypatch.setattr(
            "structorium.state.load_state", lambda _sp: calls.append(_sp) or {}
        )
    ...
    def test_falls_back_to_state_path_from_cli_main(self, monkeypatch):
        ...
        calls = []
        monkeypatch.setattr(
            "structorium.state.load_state", lambda sp: calls.append(sp) or {"ok": True}
        )
...
class TestVizWriteBehavior:
    def test_dep_graph_failure_is_best_effort(self, monkeypatch, tmp_path):
        monkeypatch.setattr(
            "structorium.app.output.visualize._collect_file_data",
        ...
        monkeypatch.setattr(
            "structorium.app.output.visualize._resolve_visualization_lang",
    ...
    def test_generate_visualization_reports_write_failure(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "structorium.app.output.visualize._build_dep_graph_for_path",
            lambda _path, _lang=None: {},
        )
        monkeypatch.setattr(
            "structorium.app.output.visualize.safe_write_text",
    ...
    def test_generate_visualization_reports_template_read_failure(
        ...
        monkeypatch.setattr(
            "structorium.app.output.visualize._get_html_template",
    ...
    def test_cmd_viz_exits_when_write_fails(self, monkeypatch, capsys, tmp_path):
        monkeypatch.setattr(
            "structorium.app.output.visualize.load_cmd_context",
            lambda _args: (tmp_path, None, {}),
        )
        monkeypatch.setattr(
            "structorium.app.output.visualize.generate_visualization",
        ...
        monkeypatch.setattr(
            "structorium.app.output.visualize.colorize",
