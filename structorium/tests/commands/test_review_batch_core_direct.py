...
from structorium.app.commands.review.batch_scoring import (
...
def test_batch_prompt_requires_score_and_finding_consistency():
    prompt = batch_core_mod.build_batch_prompt(
        repo_root=Path("/repo"),
        packet_path=Path("/repo/.structorium/review_packets/p.json"),
        batch_index=0,
        batch={
...
def test_normalize_batch_result_backfills_low_score_finding_when_enabled():
    assessments, findings, _notes, _quality = batch_core_mod.normalize_batch_result(
        payload={
            "assessments": {"logic_clarity": LOW_SCORE_FINDING_THRESHOLD - 10.0},
            "dimension_notes": {
                "logic_clarity": {
                    "evidence": [
                        "src/main.ts has repeated branch chains with dead-style fallback"
                    ],
                    "impact_scope": "module",
                    "fix_scope": "single_edit",
                    "confidence": "high",
    ...
    assert len(findings) == 1
    finding = findings[0]
    assert finding["dimension"] == "logic_clarity"
    assert finding["identifier"].startswith("auto_low_score_")
    assert finding["related_files"] == ["src/main.ts"]
    assert "evidence-backed defects" in finding["suggestion"]
