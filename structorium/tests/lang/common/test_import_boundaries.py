...
def test_detectors_layer_does_not_import_lang_layer():
    detector_dir = Path("structorium/engine/detectors")
    ...
    for py_file in sorted(detector_dir.rglob("*.py")):
        ...
        for node in ast.walk(tree):
            ...
            if module_name.startswith("structorium.languages"):
...
def test_review_cmd_uses_split_modules():
    entrypoint_src = Path("structorium/app/commands/review/entrypoint.py").read_text()
    ...
    registry_src = Path("structorium/app/commands/registry.py").read_text()
    assert "from structorium.app.commands.review import cmd_review" in registry_src


def test_scan_reporting_aggregator_uses_split_modules():
    src = Path("structorium/app/commands/scan/scan_reporting_dimensions.py").read_text()
...
def test_scan_subjective_paths_aggregator_removed():
    assert not Path(
        "structorium/app/commands/scan/scan_reporting_subjective_paths.py"
    ).exists()


def test_cli_parser_uses_group_module():
    src = Path("structorium/app/cli_support/parser.py").read_text()
