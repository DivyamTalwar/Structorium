...
class TestBuildReviewContext:
    ...
    def test_existing_findings_in_context(
        ...
        with patch("structorium.intelligence.review.context.rel") as mock_rel:
...
class TestSelectFilesForReview:
    ...
    def test_cache_skip_fresh_files(self, mock_lang, empty_state, tmp_path):
        ...
        with patch("structorium.intelligence.review.selection.rel", return_value="cached.ts"):
            state["review_cache"] = {
                "files": {
                    "cached.ts": {
    ...
    def test_cache_refresh_stale_files(self, mock_lang, empty_state, tmp_path):
        ...
        mock_lang.file_finder = MagicMock(return_value=[str(real_file)])
        state = dict(empty_state)
        with patch("structorium.intelligence.review.selection.rel", return_value="stale.ts"):
            state["review_cache"] = {
                "files": {
                    "stale.ts": {
                        "content_hash": content_hash,
    ...
    def test_content_hash_change_triggers_review(
        ...
        mock_lang.file_finder = MagicMock(return_value=[str(real_file)])
        state = dict(empty_state)
        with patch("structorium.intelligence.review.selection.rel", return_value="changed.ts"):
            state["review_cache"] = {
                "files": {
                    "changed.ts": {
...
class TestPrepareReview:
    ...
    def test_file_neighbors_included(self, mock_lang, empty_state, tmp_path):
        ...
        with (
            patch("structorium.intelligence.review.context.rel", return_value="foo.ts"),
            patch("structorium.intelligence.review.selection.rel", return_value="foo.ts"),
            patch("structorium.intelligence.review.prepare.rel", return_value="foo.ts"),
        ):
            data = prepare_review(tmp_path, mock_lang, empty_state)
        if data["files"]:
