...
from structorium import state as state_mod
from structorium.app.commands.review import batches as review_batches_mod
from structorium.app.commands.review.batch import (
    _do_run_batches,
)
from structorium.app.commands.review.import_cmd import do_import as _do_import
from structorium.app.commands.review.import_cmd import (
    do_validate_import as _do_validate_import,
)
from structorium.app.commands.review.prepare import do_prepare as _do_prepare
from structorium.app.commands.review.runtime import setup_lang_concrete as _setup_lang
from structorium.engine.policy.zones import Zone, ZoneRule
from structorium.intelligence.review import (
    import_holistic_findings,
    import_review_findings,
)
from structorium.intelligence.review.importing.per_file import update_review_cache
from structorium.state import empty_state as build_empty_state
from structorium.tests.review.shared_review_fixtures import (
    _as_review_payload,
    prepare_review,
)
...
class TestCmdReviewPrepare:
    def test_do_prepare_writes_query_json(
        ...
        with (
            patch(
                "structorium.app.commands.review.runtime.setup_lang_concrete",
                return_value=(mock_lang_with_zones, file_list),
            ),
            patch("structorium.app.commands.review.prepare.write_query", capture_query),
        ):
            _do_prepare(
                args,
                empty_state,
    ...
    def test_do_import_saves_state(self, empty_state, tmp_path):
        ...
        with patch("structorium.state.save_state", mock_save):
    ...
    def test_do_prepare_prints_narrative_reminders(self, mock_lang_with_zones, empty_state, tmp_path, capsys):
        ...
        with patch(
            ...
            return_value=(mock_lang_with_zones, file_list),
             ...
             patch("structorium.app.commands.review.prepare.write_query", lambda _data: None), \
             patch("structorium.intelligence.narrative.compute_narrative", _fake_narrative):
            _do_prepare(
                args,
                empty_state,
                mock_lang_with_zones,
    ...
    def test_do_prepare_uses_configured_batch_file_limit(
        ...
        with (
            ...
            patch(
                "structorium.app.commands.review.prepare.review_mod.prepare_holistic_review",
    ...
    def test_do_import_fails_closed_on_skipped_findings(self, empty_state, tmp_path):
        ...
        with patch("structorium.state.save_state") as mock_save:
            with pytest.raises(SystemExit):
    ...
    def test_do_run_batches_dry_run_generates_packet_and_prompts(
        ...
        review_packet_dir = tmp_path / ".structorium" / "review_packets"
        runs_dir = tmp_path / ".structorium" / "subagents" / "runs"
        ...
        with (
            ...
            patch(
                "structorium.app.commands.review.batch.review_mod.prepare_holistic_review",
                return_value=prepared,
            ),
            patch(
                "structorium.app.commands.review.prepare.write_query",
            ),
            patch(
                "structorium.app.commands.review.batch.PROJECT_ROOT",
                tmp_path,
            ),
            patch(
                "structorium.app.commands.review.batch.REVIEW_PACKET_DIR",
                review_packet_dir,
            ),
            patch(
                "structorium.app.commands.review.batch.SUBAGENT_RUNS_DIR",
                runs_dir,
            ),
            patch(
                "structorium.app.commands.review.batch._do_import",
            ) as mock_import,
        ):
            _do_run_batches(
    ...
    def test_do_run_batches_merges_outputs_and_imports(self, empty_state, tmp_path):
        ...
        with (
            patch(
                "structorium.app.commands.review.batch.subprocess.run",
    ...
    def test_do_run_batches_uses_task_map_for_execute_batches(
        ...
        with (
            ...
            patch(
                "structorium.app.commands.review.batch.runner_helpers_mod.execute_batches",
                side_effect=fake_execute_batches,
            ),
            patch(
                "structorium.app.commands.review.batch.runner_helpers_mod.collect_batch_results",
                return_value=(
                    [
                        {
                            "assessments": {"mid_level_elegance": 78.0},
    ...
    def test_run_codex_batch_returns_127_when_runner_missing(self, tmp_path):
        from structorium.app.commands.review import runner_helpers as runner_helpers_mod
    ...
    def test_run_codex_batch_stall_recovery_from_output_file(self, tmp_path):
        ...
        with patch(
            "structorium.app.commands.review.runner_helpers.codex_batch_command",
    ...
    def test_do_run_batches_scan_after_import_exits_on_failed_followup(
        ...
        with (
            ...
            patch(
                "structorium.app.commands.review.batch._merge_batch_results",
                return_value={"assessments": {}, "dimension_notes": {}, "findings": []},
            ),
            patch(
                "structorium.app.commands.review.runner_helpers.run_followup_scan",
                return_value=7,
            ),
        ):
            with pytest.raises(SystemExit) as exc_info:
