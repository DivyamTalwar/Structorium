...
import structorium.core._internal.text_utils as _utils_text_mod
import structorium.file_discovery as _file_discovery_mod
from structorium import utils as _u
from structorium.engine.detectors.review_coverage import detect_holistic_review_staleness
from structorium.intelligence.narrative.core import _count_open_by_detector
...
from structorium.intelligence.review._context.patterns import (
    extract_imported_names,
)
from structorium.intelligence.review._prepare.helpers import (
    HOLISTIC_WORKFLOW as _HOLISTIC_WORKFLOW,
)
from structorium.intelligence.review.context import file_excerpt
from structorium.intelligence.review.prepare import HolisticReviewPrepareOptions
from structorium.intelligence.review.prepare_batches import (
    batch_concerns as _batch_concerns,
    build_investigation_batches as _build_investigation_batches,
)
from structorium.intelligence.review.prepare_batches import filter_batches_to_dimensions
from structorium.scoring import (
    HOLISTIC_POTENTIAL,
    detector_pass_rate,
)
from structorium.state import empty_state, path_scoped_findings


@pytest.fixture
def patch_project_root(monkeypatch):
    """Patch PROJECT_ROOT via RuntimeContext so all consumers see the override."""
    from structorium.core.runtime_state import current_runtime_context
...
class TestPrepareHolisticReview:
    ...
    def test_prepare_holistic_review_applies_max_files_to_concern_batch(
        ...
        monkeypatch.setattr(
            "structorium.engine.concerns.generate_concerns",
...
class TestImportHolisticFindings:
    ...
    def test_reviewed_files_refreshes_per_file_cache(self, tmp_path):
        ...
        from structorium.core.runtime_state import RuntimeContext, runtime_scope
        ctx = RuntimeContext(project_root=tmp_path)
        with runtime_scope(ctx), \
             patch("structorium.intelligence.review.importing.holistic.PROJECT_ROOT", tmp_path):
    ...
    def test_reviewed_files_auto_resolves_per_file_coverage_markers(self, tmp_path):
        ...
        with runtime_scope(ctx), patch(
            "structorium.intelligence.review.importing.holistic.PROJECT_ROOT", tmp_path
        ):
...
class TestNewInvestigationBatches:
    ...
    def test_governance_batch_generated_when_reference_files_exist(self, tmp_path):
        ...
        assert "structorium/README.md" in governance["files_to_read"]
