...
def compute_tool_hash(*, tool_dir: Path | None = None) -> str:
    """Compute a content hash of all .py files in the structorium package."""
...
def check_tool_staleness(state: dict, *, tool_dir: Path | None = None) -> str | None:
    ...
    if current != stored:
        ...
        return (
            ...
            "Consider re-running: structorium scan"
        )
    if unreadable_files > 0:
        return (
...
_NEEDS_RESCAN_WARNING = (
    "Config changed — scores may be stale. Run: structorium scan"
)
