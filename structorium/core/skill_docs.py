...
SKILL_VERSION_RE = re.compile(r"<!--\s*structorium-skill-version:\s*(\d+)\s*-->")
SKILL_OVERLAY_RE = re.compile(r"<!--\s*structorium-overlay:\s*(\w+)\s*-->")

SKILL_BEGIN = "<!-- structorium-begin -->"
SKILL_END = "<!-- structorium-end -->"

# Locations where the skill doc might be installed, relative to project root.
SKILL_SEARCH_PATHS = (
    ".claude/skills/structorium/SKILL.md",
    ".opencode/skills/structorium/SKILL.md",
    "AGENTS.md",
    "CLAUDE.md",
    ".cursor/rules/structorium.md",
...
SKILL_TARGETS: dict[str, tuple[str, str, bool]] = {
    "claude": (".claude/skills/structorium/SKILL.md", "CLAUDE", True),
    # OpenCode support added with thanks to @H3xKatana.
    "opencode": (".opencode/skills/structorium/SKILL.md", "OPENCODE", True),
    "codex": ("AGENTS.md", "CODEX", False),
    "cursor": (".cursor/rules/structorium.md", "CURSOR", True),
...
def check_skill_version() -> str | None:
    ...
    if not install or not install.stale:
        return None
    return (
        f"Your structorium skill document is outdated "
        f"(v{install.version}, current v{SKILL_VERSION}). "
        "Run: structorium update-skill"
    )
