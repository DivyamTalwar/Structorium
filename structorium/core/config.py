"""Project-wide + language-specific config (.structorium/config.json)."""
...
CONFIG_FILE = PROJECT_ROOT / ".structorium" / "config.json"
...
CONFIG_SCHEMA: dict[str, ConfigKey] = {
    ...
    "ai_enabled": ConfigKey(
        bool,
        False,
        "Enable AI graph/vector context enrichment for review packets",
    ),
    "ai_include_in_review": ConfigKey(
        bool,
        True,
        "Include AI retrieval/graph context in review packets when enabled",
    ),
    "ai_embedding_model": ConfigKey(
        str,
        "text-embedding-3-large",
        "OpenAI embedding model used for vector indexing",
    ),
    "ai_embedding_dimensions": ConfigKey(
        int,
        0,
        "Embedding dimensions override (0 = model default)",
    ),
    "ai_retrieval_top_k": ConfigKey(
        int,
        24,
        "Top-K semantic candidates fetched from vector search before rerank",
    ),
    "ai_rerank_top_n": ConfigKey(
        int,
        10,
        "Top-N candidates kept after Cohere rerank",
    ),
    "ai_chunk_chars": ConfigKey(
        int,
        1800,
        "Approximate max characters per semantic chunk",
    ),
    "ai_chunk_overlap_chars": ConfigKey(
        int,
        240,
        "Character overlap between adjacent chunks",
    ),
    "ai_cohere_rerank_model": ConfigKey(
        str,
        "rerank-v3.5",
        "Cohere rerank model name",
    ),
    "ai_turbopuffer_namespace": ConfigKey(
        str,
        "structorium-code",
        "Turbopuffer namespace used for semantic chunks",
    ),
    "ai_turbopuffer_region": ConfigKey(
        str,
        "",
        "Optional Turbopuffer region override (blank = provider default)",
    ),
    "ai_neo4j_uri": ConfigKey(
        str,
        "bolt://localhost:7687",
        "Neo4j URI for local graph storage",
    ),
    "ai_neo4j_user": ConfigKey(
        str,
        "neo4j",
        "Neo4j username",
    ),
    "ai_neo4j_database": ConfigKey(
        str,
        "neo4j",
        "Neo4j database name",
    ),
