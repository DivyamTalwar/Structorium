...
Tests cover:
  1. app.commands.resolve.render
  2. app.commands.status_parts.strict_target
  3. app.commands._show_terminal
  4. app.commands.viz_cmd
  5. app.commands.review.entrypoint
  6. app.commands.update_skill
"""
...
# ── 1. resolve/render.py ────────────────────────────────────────────────────
from app.commands.resolve.render import (
    _delta_suffix,
    _print_next_command,
...
class TestPrintResolveSummary:
    @patch("app.commands.resolve.render.colorize", side_effect=lambda t, _c: t)
...
class TestPrintScoreMovement:
    @patch("app.commands.resolve.render.state_mod")
    @patch("app.commands.resolve.render.colorize", side_effect=lambda t, _c: t)
    def test_scores_unavailable(self, _mock_colorize, mock_state, capsys):
        from state import ScoreSnapshot
...
from app.commands.status_parts.strict_target import (  # noqa: E402
...
from app.commands._show_terminal import (  # noqa: E402
...
class TestShowFixDryRunSamples:
    @patch("app.commands._show_terminal.colorize", side_effect=lambda t, _c: t)
    @patch("app.commands._show_terminal._print_fix_file_sample")
...
class TestPrintFixFileSample:
    @patch("app.commands._show_terminal.colorize", side_effect=lambda t, _c: t)
    @patch("app.commands._show_terminal.rel", side_effect=lambda p: p)
...
from app.commands import viz_cmd  # noqa: E402
from app.commands.viz_cmd import cmd_tree, cmd_viz  # noqa: E402


class TestVizCmd:
    @patch("app.commands.viz_cmd._cmd_viz")
    def test_cmd_viz_delegates(self, mock_inner):
        args = argparse.Namespace()
        cmd_viz(args)
        mock_inner.assert_called_once_with(args)

    @patch("app.commands.viz_cmd._cmd_tree")
...
class TestCmdReviewEntrypoint:
    @patch("app.commands.review.entrypoint.do_prepare")
    ...
    @patch("app.commands.review.entrypoint.do_import")
    @patch("app.commands.review.entrypoint.resolve_lang")
    @patch("app.commands.review.entrypoint.command_runtime")
    ...
    @patch("app.commands.review.entrypoint.do_validate_import")
    @patch("app.commands.review.entrypoint.resolve_lang")
    @patch("app.commands.review.entrypoint.command_runtime")
    ...
    @patch("app.commands.review.entrypoint._do_run_batches")
    @patch("app.commands.review.entrypoint.resolve_lang")
    @patch("app.commands.review.entrypoint.command_runtime")
    ...
    @patch("app.commands.review.entrypoint.do_external_submit")
    @patch("app.commands.review.entrypoint.resolve_lang")
    @patch("app.commands.review.entrypoint.command_runtime")
...
from app.commands.update_skill import (  # noqa: E402
    _build_section,
    _replace_section,
    cmd_update_skill,
    resolve_interface,
...
class TestReplaceSection:
    ...
    def test_replaces_between_markers(self):
        from utils import SKILL_BEGIN, SKILL_END
        content = f"before\n{SKILL_BEGIN}\nold content\n{SKILL_END}\nafter"
...
class TestResolveInterface:
    ...
    def test_none_with_no_install(self):
        with patch(
            "app.commands.update_skill.find_installed_skill",
            return_value=None,
        ):
            assert resolve_interface(None) is None

    def test_from_install_overlay(self):
        from utils import SkillInstall
...
class TestCmdUpdateSkill:
    @patch("app.commands.update_skill.update_installed_skill")
    @patch("app.commands.update_skill.resolve_interface", return_value="claude")
    ...
    @patch("app.commands.update_skill.colorize", side_effect=lambda t, _c: t)
    @patch("app.commands.update_skill.resolve_interface", return_value=None)
    ...
    @patch("app.commands.update_skill.colorize", side_effect=lambda t, _c: t)
    @patch("app.commands.update_skill.resolve_interface", return_value="unknown_thing")
...
class TestUpdateInstalledSkill:
    @patch("app.commands.update_skill.colorize", side_effect=lambda t, _c: t)
    @patch("app.commands.update_skill._download")
    ...
    def test_successful_dedicated_install(self, mock_download, _mock_colorize, capsys, tmp_path):
        ...
        with patch(
            "app.commands.update_skill.get_project_root",
