"""Tests for languages._framework.commands_base — command factory functions.
...
class TestMakeCmdLarge:
    ...
    @patch("languages._framework.commands_base.display_entries")
    def test_calls_detect_large_files(self, mock_display):
        ...
        with patch(
            "engine.detectors.large.detect_large_files", return_value=([], 0)
        ) as mock_detect:
            cmd(_make_args())
...
class TestMakeCmdComplexity:
    ...
    def test_calls_detect_complexity(self, mock_display):
        ...
        with patch(
            "engine.detectors.complexity.detect_complexity", return_value=([], 0)
        ) as mock_detect:
            cmd(_make_args())
            mock_detect.assert_called_once()
            mock_display.assert_called_once()
