"""Tests for cli — argument parsing, state path resolution, helpers."""
...
import app.commands.helpers.lang as lang_helpers_mod
...
from app.commands.helpers.runtime_options import (
    LangRuntimeOptionsError,
    resolve_lang_runtime_options,
)
from app.commands.helpers.score import (
    coerce_target_score,
...
from languages.csharp import CSharpConfig
...
class TestStatePath:
    def test_auto_detects_lang_when_no_state_or_lang(self):
        ...
        with patch("languages.auto_detect_lang", return_value="python"):
        ...
        with patch("languages.auto_detect_lang", return_value=None):
            result = state_path(args)
    ...
    def test_non_scan_falls_back_to_sole_existing_lang_state(
        ...
        existing.write_text("{}")
        monkeypatch.setattr("app.commands.helpers.state.PROJECT_ROOT", tmp_path)
    ...
    def test_non_scan_without_lang_uses_sole_existing_state(
        ...
        monkeypatch.setattr("app.commands.helpers.state.PROJECT_ROOT", tmp_path)
        monkeypatch.setattr(
            "app.commands.helpers.state.auto_detect_lang_name",
            lambda _args: None,
        )
...
class TestResolveDefaultPath:
    ...
    def test_review_uses_scan_path_from_state(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "app.commands.helpers.state.PROJECT_ROOT", project_root
        )

        with (
            patch("cli.state_path", return_value=tmp_path / "state.json"),
            patch("cli.load_state", return_value=saved_state),
        ):
    ...
    def test_review_falls_back_to_lang_default_when_no_scan_path(self, monkeypatch):
        ...
        with (
            patch("cli.state_path", return_value=None),
            patch("cli.load_state", return_value={}),
            patch("cli.resolve_lang") as mock_lang,
        ):
    ...
    def test_review_falls_back_when_state_load_raises(self, monkeypatch):
        ...
        with (
            patch("cli.state_path", return_value=None),
            patch("cli.load_state", side_effect=OSError("no file")),
            patch("cli.resolve_lang") as mock_lang,
        ):
            mock_lang.return_value = SimpleNamespace(default_src="src")
            args = SimpleNamespace(command="review", path=None)
    ...
    def test_non_review_command_uses_lang_default(self):
        with patch("cli.resolve_lang") as mock_lang:
            mock_lang.return_value = SimpleNamespace(default_src="src")
...
class TestResolveLang:
    ...
    def test_auto_detect_uses_path_when_it_looks_like_project_root(
        ...
        monkeypatch.setattr("utils.PROJECT_ROOT", cwd_root)
    ...
    def test_lang_config_markers_include_plugin_markers(self, monkeypatch):
        ...
        monkeypatch.setattr("languages.available_langs", lambda: ["dummy"])
        monkeypatch.setattr("languages.get_lang", lambda _name: DummyCfg())
...
class TestWriteQuery:
    def test_writes_valid_json(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.commands.helpers.query.query_file_path",
            lambda: query_file,
        )
...
class TestApplyPersistedExclusions:
    def test_cli_exclusions_applied(self, monkeypatch):
        captured = []
        monkeypatch.setattr(
            "cli.set_exclusions", lambda pats: captured.extend(pats)
        )
    ...
    def test_no_exclusions_does_nothing(self, monkeypatch):
        called = []
        monkeypatch.setattr(
            "cli.set_exclusions", lambda pats: called.append(pats)
        )
