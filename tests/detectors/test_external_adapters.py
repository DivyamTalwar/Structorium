...
# ── Knip adapter ────────────────────────────────────────────────────────────
from languages.typescript.detectors.knip_adapter import detect_with_knip
...
from languages.python.detectors.ruff_smells import detect_with_ruff_smells  # noqa: E402,I001
...
from languages.python.detectors.bandit_adapter import (  # noqa: E402
    _to_security_entry,
    detect_with_bandit,
)
...
class TestBanditExcludeIntegration:
    ...
    def test_detect_lang_security_passes_exclusions_to_bandit(self):
        ...
        def _fake_bandit(path, zone_map, **kwargs):
            from languages.python.detectors.bandit_adapter import (
                BanditRunStatus,
                BanditScanResult,
            )
        ...
        with patch(
            "languages.python.detect_with_bandit", _fake_bandit
        ), patch(
            "languages.python.collect_exclude_dirs",
...
from engine.detectors.jscpd_adapter import (  # noqa: E402
    _parse_jscpd_report,
    detect_with_jscpd,
)
...
class TestJscpdAdapter:
    ...
    def test_detect_command_includes_artifact_ignores(self, tmp_path):
        ...
        with patch("subprocess.run", side_effect=_fake_run), patch(
            "engine.detectors.jscpd_adapter.collect_exclude_dirs",
            return_value=fake_dirs,
        ), patch(
            "engine.detectors.jscpd_adapter.get_exclusions",
            return_value=(),
        ), patch(
...
from languages.python.detectors.import_linter_adapter import (  # noqa: E402
...
from core.source_discovery import collect_exclude_dirs  # noqa: E402


class TestCollectExcludeDirs:
    def test_returns_absolute_paths(self, tmp_path):
        with patch(
            "core.source_discovery.get_exclusions", return_value=()
        ):
...
from languages.python.detectors.unused import detect_unused  # noqa: E402
...
class TestRuffSmellsExcludeFlag:
    ...
    def test_ruff_smells_passes_exclude_flag(self, tmp_path):
        ...
        with patch("subprocess.run", side_effect=_capture_run), patch(
            "languages.python.detectors.ruff_smells._collect_exclude_dirs",
            return_value=fake_dirs,
        ):
...
class TestRuffUnusedExcludeFlag:
    ...
    def test_ruff_unused_passes_exclude_flag(self, tmp_path):
        ...
        with patch("subprocess.run", side_effect=_capture_run), patch(
            "languages.python.detectors.unused._collect_exclude_dirs",
            return_value=fake_dirs,
        ), patch(
            "languages.python.detectors.unused.find_py_files",
            return_value=[],
        ):
