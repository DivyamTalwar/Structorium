...
import core._internal.text_utils as _utils_text_mod
import file_discovery as _file_discovery_mod
import utils as _u
from engine.detectors.review_coverage import detect_holistic_review_staleness
from intelligence.narrative.core import _count_open_by_detector
...
from intelligence.review._context.patterns import (
    extract_imported_names,
)
from intelligence.review._prepare.helpers import (
    HOLISTIC_WORKFLOW as _HOLISTIC_WORKFLOW,
)
from intelligence.review.context import file_excerpt
from intelligence.review.prepare import HolisticReviewPrepareOptions
from intelligence.review.prepare_batches import (
    batch_concerns as _batch_concerns,
    build_investigation_batches as _build_investigation_batches,
)
from intelligence.review.prepare_batches import filter_batches_to_dimensions
from scoring import (
    HOLISTIC_POTENTIAL,
    detector_pass_rate,
)
from state import empty_state, path_scoped_findings


@pytest.fixture
def patch_project_root(monkeypatch):
    """Patch PROJECT_ROOT via RuntimeContext so all consumers see the override."""
    from core.runtime_state import current_runtime_context
...
class TestPrepareHolisticReview:
    ...
    def test_prepare_holistic_review_applies_max_files_to_concern_batch(
        ...
        monkeypatch.setattr(
            "engine.concerns.generate_concerns",
...
class TestImportHolisticFindings:
    ...
    def test_reviewed_files_refreshes_per_file_cache(self, tmp_path):
        ...
        with runtime_scope(ctx), \
             patch("intelligence.review.importing.holistic.PROJECT_ROOT", tmp_path):
    ...
    def test_reviewed_files_auto_resolves_per_file_coverage_markers(self, tmp_path):
        ...
        with runtime_scope(ctx), patch(
            "intelligence.review.importing.holistic.PROJECT_ROOT", tmp_path
        ):
...
class TestNewInvestigationBatches:
    ...
    def test_governance_batch_generated_when_reference_files_exist(self, tmp_path):
        ...
        assert "README_TECHNICAL.md" in governance["files_to_read"]
