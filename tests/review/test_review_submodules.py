...
from intelligence.review._prepare.remediation_engine import (
    empty_plan as _empty_plan,
)
from intelligence.review.importing.holistic import (
    import_holistic_findings,
    update_holistic_review_cache,
)
from intelligence.review.importing.per_file import (
    import_review_findings,
    parse_per_file_import_payload,
    update_review_cache,
)
from intelligence.review.importing.shared import (
    extract_reviewed_files,
    store_assessments,
)
from intelligence.review.prepare import (
...
from intelligence.review.remediation import (
    generate_remediation_plan,
)
from intelligence.review.selection import (
    LOW_VALUE_NAMES,
...
from state import empty_state as build_empty_state
...
class TestGetFileFindings:
    ...
    def test_finds_matching(self, empty_state):
        ...
        with patch("intelligence.review.selection.rel", side_effect=lambda x: x):
...
class TestComputeReviewPriority:
    def test_tiny_file_filtered(self, mock_lang, empty_state):
        with (
            patch("intelligence.review.selection.rel", return_value="tiny.ts"),
            patch("intelligence.review.selection.read_file_text", return_value="x\n" * 5),
        ):
    ...
    def test_normal_file_gets_score(self, mock_lang, empty_state):
        content = "line\n" * 100
        with (
            patch("intelligence.review.selection.rel", return_value="src/app.ts"),
            patch("intelligence.review.selection.read_file_text", return_value=content),
        ):
    ...
    def test_low_value_penalty(self, mock_lang, empty_state):
        content = "line\n" * 100
        with (
            patch("intelligence.review.selection.rel") as mock_rel,
            patch("intelligence.review.selection.read_file_text", return_value=content),
        ):
...
class TestSelectFilesForReview:
    ...
    def test_skips_cached_fresh(self, mock_lang, empty_state):
        ...
        with (
            patch("intelligence.review.selection.rel", return_value="src/a.ts"),
            patch("intelligence.review.selection.hash_file", return_value=content_hash),
            patch(
                "intelligence.review.selection._compute_review_priority", return_value=10
            ),
        ):
...
class TestRelList:
    def test_set_input(self):
        with patch("intelligence.review.prepare.rel", side_effect=lambda x: x):
...
class TestBuildFileRequests:
    def test_basic(self, mock_lang, empty_state):
        with (
            patch(
                "intelligence.review.prepare.read_file_text", return_value="line1\nline2"
            ),
            patch("intelligence.review.prepare.rel", return_value="src/a.ts"),
            patch("intelligence.review.prepare.abs_path", side_effect=lambda x: x),
        ):
    ...
    def test_skips_unreadable(self, mock_lang, empty_state):
        with (
            patch("intelligence.review.prepare.read_file_text", return_value=None),
            patch("intelligence.review.prepare.abs_path", side_effect=lambda x: x),
        ):
...
class TestPrepareReview:
    def test_returns_expected_keys(self, mock_lang, empty_state):
        with (
            patch("intelligence.review.prepare.build_review_context") as mock_ctx,
            patch("intelligence.review.prepare.select_files_for_review", return_value=[]),
            patch("intelligence.review.prepare._build_file_requests", return_value=[]),
            patch("intelligence.review.prepare.serialize_context", return_value={}),
            patch("intelligence.review.prepare.count_fresh", return_value=0),
            patch("intelligence.review.prepare.count_stale", return_value=0),
        ):
...
class TestPrepareHolisticReview:
    def test_returns_expected_keys(self, mock_lang, empty_state):
        with (
            patch("intelligence.review.prepare.build_review_context") as mock_review_ctx,
            patch("intelligence.review.prepare.build_holistic_context", return_value={}),
            patch("intelligence.review.prepare.serialize_context", return_value={}),
            patch(
                "intelligence.review.prepare._build_investigation_batches",
                return_value=[],
...
class TestGenerateRemediationPlan:
    ...
    def test_writes_to_file(self, empty_state, tmp_path):
        ...
        with patch("intelligence.review._prepare.remediation_engine.safe_write_text") as mock_write:
