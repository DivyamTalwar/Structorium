...
def _print_plan_agent_block(plan: dict, *, header: str = "  AGENT PLAN:") -> None:
    ...
    print(colorize("  Next command: `structorium next`", "dim"))
    print(colorize("  View plan: `structorium plan`", "dim"))
...
def print_ranked_actions(
    ...
    for action in ranked[:limit]:
        ...
        if cluster_count:
            print(colorize(f"    - {detector}: {count} open in {cluster_count} cluster(s) — `structorium next`", "dim"))
        else:
            command = action.get("command", "structorium next")
