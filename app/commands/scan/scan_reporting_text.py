...
def build_workflow_guide(attest_example: str) -> str:
    ...
    return dedent(
        ...
        **inner** (plan → fix next → update plan → repeat until plan clear).
        ...
        1. **Follow `next`**: `structorium next` — the single source of truth for what to work on.
           It respects the plan, surfaces auto-clustered batches, and tells you exactly what to do.
        2. **Fix & resolve**: Fix the issue, then:
           `structorium plan done "<id>" --note "<what you did>" --attest "{attest_example}"`
           Required attestation keywords: 'I have actually' and 'not gaming'.
        3. **Plan strategically**: `structorium plan` — reorder, cluster related issues, defer low-value work.
           Think about sequencing: what unblocks the most? What cascades? What can be batched?
        4. **Run auto-fixers** (if available): `structorium fix <fixer> --dry-run` to preview, then apply.
        5. **Rescan**: `structorium scan --path <path>` — verify improvements, catch cascading effects.
        6. **Subjective review**: `structorium review --run-batches --runner codex --parallel --scan-after-import`
           (or `review --prepare` → review → import for manual path).
        7. **Check progress**: `structorium status` — dimension scores dashboard.
        ...
        - **Wontfix**: Intentional patterns, false positives →
          `structorium plan skip --permanent "<id>" --note "<why>" --attest "{attest_example}"`
        - **Batch wontfix**: Multiple intentional patterns →
          `structorium plan skip --permanent "<detector>::*::<category>" --note "<why>" --attest "{attest_example}"`
