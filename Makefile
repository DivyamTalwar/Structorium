...
package-smoke: install-ci-tools
	...
	. .pkg-smoke/bin/activate && \
		...
		WHEEL=$$(ls -t dist/structorium-*.whl | head -n 1) && \
		python -m pip install "$$WHEEL[full]" && \
		python -c "import importlib.metadata as m,sys; extras=set(m.metadata('structorium').get_all('Provides-Extra') or []); required={'full','treesitter','python-security','scorecard'}; missing=required-extras; print('missing extras metadata:', sorted(missing)) if missing else None; sys.exit(1 if missing else 0)" && \
		structorium --help > /dev/null
	rm -rf .pkg-smoke
