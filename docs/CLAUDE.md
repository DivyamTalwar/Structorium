...
1. **Preferred**: `structorium review --run-batches --runner codex --parallel --scan-after-import` — does everything in one command.
2. **Claude cloud path**: `structorium review --external-start --external-runner claude` → use generated `claude_launch_prompt.md` + `review_result.template.json` → run printed `structorium review --external-submit --session-id <id> --import <file>`.
3. **Manual path**: split dimensions across N subagents (one message, multiple Task calls), merge outputs, then `structorium review --import findings.json`.
...
3. For blind reviews, consume `.structorium/review_packet_blind.json` instead of full `query.json`.
